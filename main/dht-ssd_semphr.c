...
void DHT_task(void *pvParameter) {
  ...
  while (1) {
    ...
    if (res != DHT_OK) {
      ...
      ESP_LOGI(TAG, "DHT_task() trying to take Semaphore");
      ...
      ESP_LOGI(TAG, "Failed to take Semaphore!!");
...
int app_main() {
  ...
  while (1) {
    ESP_LOGI(TAG, "app_main() trying to take Semaphore");

    if (xSemaphoreTake(semPtr, portMAX_DELAY) == pdPASS) {
      ESP_LOGI(TAG, "Acquired Semaphore.");
